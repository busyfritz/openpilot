...
class UnifiedLabel(Widget):
  ...
  def _render(self, _):
    ...
    for idx, (line, size, emojis) in enumerate(zip(visible_lines, visible_sizes, visible_emojis, strict=True)):
      ...
      # Draw 2nd instance for scrolling
      if self._needs_scroll and self._scroll_active and self._scroll_state != ScrollState.STARTING:
    ...
    if self._needs_scroll:
      ...
      if self._scroll_active and self._scroll_state != ScrollState.STARTING:
