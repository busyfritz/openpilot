...
class NavScroller(NavWidget):
  ...
  def _draw_page_slider(self):
    sc = self._scroller
    if not sc._horizontal:
      return
    max_off = sc._content_size - sc.rect.width
    if max_off <= 1:
      return
    progress = max(0.0, min(1.0, (-sc.scroll_panel.get_offset()) / max_off))

    from openpilot.selfdrive.ui.iqpilot.theme import NeonTheme
    c = NeonTheme.glow(255)
    r, g, b = c.r, c.g, c.b
    glow_w = self.PAGE_SLIDER_GLOW_W
    h = self.PAGE_SLIDER_GLOW_H
    end = self.PAGE_SLIDER_END_TAPER
    left = self._rect.x - end
    right = self._rect.x + self._rect.width - glow_w + end
    x = int(left + progress * (right - left))
    bottom = int(self._rect.y + self._rect.height)

    rl.draw_rectangle_gradient_v(x, bottom - h, glow_w, h, rl.Color(r, g, b, 0), rl.Color(r, g, b, self.PAGE_SLIDER_PEAK))
  ...
  def _draw_edge_fades(self):
    fw = self.PAGE_EDGE_FADE_W
    x, y = int(self._rect.x), int(self._rect.y)
    w, h = int(self._rect.width), int(self._rect.height)
