...
class NavWidget(Widget, abc.ABC):
  ...
  def covers_below(self) -> bool:
    # Full-screen opaque page: never draw the widgets underneath (even mid-swipe), so the
    # page below stays hidden behind black while this one slides away instead of showing
    # through. The revealed page is settled to rest on pop (see settle_to_top), so it does
    # not bounce/slide in when it reappears.
    return True
