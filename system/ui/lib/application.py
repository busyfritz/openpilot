...
class GuiApplication(GuiApplicationExt):
  ...
  def init_window(self, title: str, fps: int = _DEFAULT_FPS):
    with self._startup_profile_context():
      ...
      # On mici (comma four) the panel refreshes slightly above 60Hz; pinning raylib to a 60fps
      # sleep cap drifts against vblank and periodically swallows frames onroad. Pass 0 so the
      # display's vblank drives cadence instead (stock PR #37970). _target_fps still holds the
      # real fps below so the should-render sleep path keeps its timing.
