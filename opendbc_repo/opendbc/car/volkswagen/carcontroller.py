...
class CarController(CarControllerBase):
  def __init__(self, dbc_names, CP, CP_IQ):
    ...
    self.motor1_pedalwert_last = None
  ...
  def update(self, CC, CC_IQ, CS, now_nanos):
    ...
    if self.CP.flags & VolkswagenFlags.PQ:
      if self.frame % 2 == 0:
        ...
        can_sends.append(self.CCS.create_blinker_control(self.packer_pt, self._pt_tx_bus, leftBlinker, rightBlinker))
    ...
    if self.CP.flags & VolkswagenFlagsIQ.IQ_PQ_ACC_FTS_EPB:
      motor1_new = CS.motor1_pedalwert != self.motor1_pedalwert_last
      motor1_resume_active = (
        self.CP.openpilotLongitudinalControl
        and CC.cruiseControl.resume
        and CS.out.cruiseState.standstill
      )
      if motor1_new or motor1_resume_active:
        can_sends.append(self.CCS.create_motor1_resume(
          self.packer_pt, self.CAN.aux, CS.motor1_stock, resume=motor1_resume_active))
    ...
    self.motor1_pedalwert_last = CS.motor1_pedalwert
    self.frame += 1
