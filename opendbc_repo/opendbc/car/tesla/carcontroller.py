...
from opendbc.car.tesla.values import CarControllerParams, DBC
...
class CarController(CarControllerBase):
  def __init__(self, dbc_names, CP, CP_IQ):
    ...
    self.packer_vehicle = CANPacker(dbc_names[Bus.adas]) if Bus.adas in dbc_names else None
  ...
  def update(self, CC, CC_IQ, CS, now_nanos):
    ...
    if self.packer_vehicle is not None and getattr(CS, 'das_body_controls', None) is not None:
      body_counter = int(CS.das_body_controls["DAS_bodyControlsCounter"])
      body_send_ready = body_counter != self.body_controls_counter_last
      if body_send_ready:
        blinker_active = CS.out.leftBlinker or CS.out.rightBlinker
        ...
        can_sends.append(self.tesla_can.create_body_controls(self.packer_vehicle, CS.das_body_controls,
