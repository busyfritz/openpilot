...
class CarState(CarStateBase, CarStateExt):
  def __init__(self, CP, CP_IQ):
    ...
    # HW4.5 shares the party and autopilot buses; DI_state/DAS_status are absent and
    # UI_warning uses a different layout, so those reads are skipped to keep canValid.
    self.hw45 = bool(CP.flags & TeslaFlags.HW45_SHARED_BUS)

    # HW4.5 counter-spoof: track stock AP counters on party bus
    self.stock_steer_counter = -1
    self.stock_eac_counter = -1
  ...
  # Vehicle speed
    ...
    # DI_state is absent on HW4.5; cruise state/engagement is not yet available there.
    if not self.hw45:
    ...
    # UI_warning uses a different layout on HW4.5 (8B vs 7B); skip these reads there.
    if not self.hw45:
      # Doors
    ...
    # Blindspot — DAS_status is absent on HW4.5
    if not self.hw45:
    ...
    if not (self.CP.flags & TeslaFlags.MISSING_DAS_SETTINGS) and not self.hw45:
    ...
    # HW4.5 shared bus: track stock AP counters on party bus for counter-spoofing
    if self.CP.flags & TeslaFlags.HW45_SHARED_BUS:
      self.stock_steer_counter = int(cp_party.vl["DAS_steeringControl"]["DAS_steeringControlCounter"])
      self.stock_eac_counter = int(cp_party.vl["APS_eacMonitor"]["APS_eacMonitorCounter"])
