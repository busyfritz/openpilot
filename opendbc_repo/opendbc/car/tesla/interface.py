...
class CarInterface(CarInterfaceBase):
  ...
  def _get_params(ret: structs.CarParams, candidate, fingerprint, car_fw, alpha_long, is_release, docs) -> structs.CarParams:
    ...
    # HW4.5 shared bus detection: DAS_steeringControl (0x488) natively on bus 0
    # means party and autopilot_party are the same physical CAN bus
    if 0x488 in fingerprint[CANBUS.party] and 0x286 not in fingerprint[CANBUS.party]:
      ret.flags |= TeslaFlags.HW45_SHARED_BUS.value
      ret.safetyConfigs[0].safetyParam |= TeslaSafetyFlags.HW45_SHARED_BUS.value
