...
class TeslaCAN:
  ...
  def create_steering_control(self, angle, enabled, control_type, counter=None):
    values = {
    ...
    if counter is not None:
      values["DAS_steeringControlCounter"] = counter
  ...
  def create_steering_allowed(self, counter=None):
    values = {
      "APS_eacAllow": 1,
    }
    if counter is not None:
      values["APS_eacMonitorCounter"] = counter
