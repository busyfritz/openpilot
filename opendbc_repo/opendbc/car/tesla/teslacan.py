...
class TeslaCAN:
  ...
  def create_body_controls(self, packer_vehicle, stock_values, left_blinker, right_blinker):
    ...
    turn_req = 1 if left_blinker else (2 if right_blinker else 0)
    turn_reason = 4 if turn_req else 0  # 4 = navigation turn
    values["DAS_turnIndicatorRequest"] = turn_req
    values["DAS_turnIndicatorRequestReason"] = turn_reason

    counter = (int(stock_values["DAS_bodyControlsCounter"]) + 1) % 16
    values["DAS_bodyControlsCounter"] = counter

    return packer_vehicle.make_can_msg("DAS_bodyControls", CANBUS.vehicle, values)
