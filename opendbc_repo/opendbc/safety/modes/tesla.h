...
static bool tesla_tx_hook(const CANPacket_t *msg) {
  ...
  // DAS_bodyControls: blinker control on vehicle bus
  if ((msg->addr == 0x3E9U) && (msg->bus == 1U)) {
    int turn_request = (msg->data[1]) & 0x03U;
    // Only allow valid turn indicator values: 0=off, 1=left, 2=right
    if (turn_request > 2) {
      violation = true;
    }
    // No hazard light control
    int hazard_request = (msg->data[0] >> 2) & 0x03U;
    if (hazard_request != 0) {
      violation = true;
    }
  }
...
static bool tesla_fwd_hook(int bus_num, int addr) {
  ...
  if (bus_num == 2) {
    if (!tesla_summon) {
      ...
      // DAS_bodyControls: block forwarding to vehicle bus so we can MITM blinker control
      if (tesla_has_vehicle_bus && (addr == 0x3E9)) {
        block_msg = true;
      }
    }
  }
