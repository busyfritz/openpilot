...
#define TESLA_HW45_RX_CHECKS \
  {.msg = {{0x2b9, 0, 8, 25U, .max_counter = 7U, .ignore_quality_flag = true}, { 0 }, { 0 }}},    /* DAS_control (on bus 0, shared bus) */    \
  {.msg = {{0x488, 0, 4, 50U, .max_counter = 15U, .ignore_quality_flag = true}, { 0 }, { 0 }}},   /* DAS_steeringControl (on bus 0) */        \
  ...
  /* DI_state (0x286) is MISSING on HW4.5 -- skip cruise check for now */                                                                       \

static bool tesla_longitudinal = false;
static bool tesla_fsd_14 = false;
static bool tesla_stock_aeb = false;
static bool tesla_hw45_shared_bus = false;
...
static void tesla_rx_hook(const CANPacket_t *msg) {
  ...
  if (msg->bus == 0U) {
    ...
    // Cruise and Autopark/Summon state (not available on HW4.5 — DI_state is missing)
    if (!tesla_hw45_shared_bus && (msg->addr == 0x286U)) {
      // Autopark state
  ...
  // On HW4.5 shared bus, stock AP messages appear on bus 0 instead of bus 2
  uint8_t ap_bus = tesla_hw45_shared_bus ? 0U : 2U;
...
static bool tesla_fwd_hook(int bus_num, int addr) {
  ...
  // HW4.5 shared bus: no forwarding needed, both buses are the same physical wire
  if (tesla_hw45_shared_bus) {
    return false;
  }
...
static safety_config tesla_init(uint16_t param) {
  ...
  // HW4.5 shared bus: no relay check since stock AP is on the same physical bus
  static const CanMsg TESLA_HW45_TX_MSGS[] = {
    {0x488, 0, 4, .check_relay = false},  // DAS_steeringControl (counter-spoofed)
    {0x2b9, 0, 8, .check_relay = false},  // DAS_control (for cancel)
    {0x27D, 0, 3, .check_relay = false},  // APS_eacMonitor (counter-spoofed)
  };

  const uint16_t TESLA_FLAG_FSD_14 = 2;
  tesla_fsd_14 = GET_FLAG(param, TESLA_FLAG_FSD_14);

  const uint16_t TESLA_FLAG_HW45_SHARED_BUS = 4;
  tesla_hw45_shared_bus = GET_FLAG(param, TESLA_FLAG_HW45_SHARED_BUS);
  ...
  // HW4.5: DI_state doesn't exist, so no summon detection — assume safe
  tesla_summon = !tesla_hw45_shared_bus;
  ...
  static RxCheck tesla_hw45_rx_checks[] = {
    TESLA_HW45_RX_CHECKS
  };

  safety_config ret;
  if (tesla_hw45_shared_bus) {
    SET_TX_MSGS(TESLA_HW45_TX_MSGS, ret);
    SET_RX_CHECKS(tesla_hw45_rx_checks, ret);
