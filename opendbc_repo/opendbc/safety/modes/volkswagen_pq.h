...
static safety_config volkswagen_pq_init(uint16_t param) {
  ...
  // All RX checks on bus 1 (ptCAN). Messages are present on both bus 0 and bus 1 on ECAN gateway
  // cars (J533 bridges ptCAN to ECAN). On lowline non-ECAN cars bus 1 is the only active bus.
