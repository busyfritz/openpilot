...
class AugmentedRoadView(CameraView):
  ...
  def _reblacken_border_edges(self):
    # The teal AOL frame should keep BLACK rounded corners (like stock) and a black right edge on
    # the confidence-ball side — teal only on the top/left/bottom straight edges. Re-draw the black
    # border clipped to those regions; it's the same draw call, so the black exactly re-covers the
    # teal there (no separate solid rect that would cut into the camera / turn-signal).
    cr = self._content_rect
    # corner-box size: rounded-corner radius + line thickness (+margin) fully covers the corner arc
