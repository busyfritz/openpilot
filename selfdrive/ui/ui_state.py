  ...
  def _update_status(self) -> None:
    if self.started and self.sm.updated["selfdriveState"]:
      ...
      if computed_status != UIStatus.DISENGAGED:
        ...
        # Still actually engaged (selfdrive or AOL): ignore a transient IQ/selfdrive status race
        # and hold the last engaged status so the border doesn't flicker mid-engagement.
        if self._last_non_disengaged_status != UIStatus.DISENGAGED:
          self.status = self._last_non_disengaged_status
        else:
          self.status = UIStatus.ENGAGED if ss.enabled else UIStatus.DISENGAGED
      else:
        # Truly disengaged (neither selfdrive nor AOL engaged): drop to disengaged immediately so
        # the engagement border turns off instantly, like stock. (No timed linger — that delayed
        # the border ~up to 2s after disengaging on every platform.)
