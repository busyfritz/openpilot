...
class VCruiseHelper(VCruiseHelperIQ):
  ...
  def initialize_v_cruise(self, CS, experimental_mode: bool, iq_dynamic_mode: bool) -> None:
    ...
    # IQE2ESetSpeedMode: 0 = default (105 kph / 65 mph), 1 = use current vehicle speed on SET.
    # Only applies to full IQ.Pilot e2e mode (not IQ.Dynamic or IQ.Standard).
    if initial_experimental_mode:
      e2e_mode = Params().get("IQE2ESetSpeedMode", return_default=True)
      use_current_speed = int(e2e_mode or 0) == 1
      initial = V_CRUISE_MIN if use_current_speed else V_CRUISE_INITIAL_EXPERIMENTAL_MODE
    else:
