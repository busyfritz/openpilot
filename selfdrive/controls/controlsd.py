...
class Controls(ControlsExt):
  def __init__(self) -> None:
    ...
    self.sm = messaging.SubMaster(['liveDelay', 'liveParameters', 'liveTorqueParameters', 'modelV2', 'selfdriveState',
                                   ...
                                   'driverAssistance', 'liveDelay', 'liveCurvatureParameters'] + self.sm_services_ext,
                                  poll='selfdriveState')
    ...
    # PT2 smooth-steer filter on the desired curvature command (anti-wobble). Default on for
    # curvature cars (VW MEB) unless the user has explicitly toggled it off.
    ...
    # CurvatureD: live-learned curvature correction (opt-in via EnableCurvatureD; the learner
    # process only runs when enabled). Controller is inert until liveCurvatureParameters are valid.
    self.curvatured = CurvatureDController() if self.is_curvature_car else None
  ...
  def update(self):
    self.sm.update(15)
    if self.curvatured is not None and self.sm.updated["liveCurvatureParameters"]:
      self.curvatured.update_live_params(self.sm['liveCurvatureParameters'])
  ...
  def state_control(self):
    ...
    # PT2 low-pass the desired curvature to damp lateral wobble (VW MEB). Seed the filter to the
    # current curvature while inactive so re-engaging doesn't step from zero.
    if self.enable_smooth_steer and CC.latActive:
    ...
    # CurvatureD: add the live-learned curvature correction to the command (opt-in, inert until
    # the learner publishes valid params).
    if self.curvatured is not None and self.enable_curvatured and CC.latActive and \
       self.sm.all_checks(['liveCurvatureParameters']):
      actuators.curvature = float(self.desired_curvature + self.curvatured.get_correction(self.desired_curvature, CS.vEgo))
