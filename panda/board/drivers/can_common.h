...
// Per-bus latch: set true when openpilot explicitly requests a CAN FD data bitrate for a bus
// (USB/SPI control 0xf9, see main_comms.h). can_init_all() runs on every safety-model change
// (including under SAFETY_SILENT at boot), and by default zeroes can_data_speed so classic-CAN
// ports come up classic. A requested bus keeps its FD data bitrate across those re-inits, which
// is what lets CAN FD stay up before the car safety model is known (e.g. VW MEB ignition, where
// the 0x3C0 ignition message rides the FD powertrain bus and must be readable while still in
// noOutput). Buses that were never requested reset to 0U exactly as before — other cars are
// untouched, and the FD scoping lives in pandad (which only requests FD for VW MEB/MQBevo).
bool canfd_requested[PANDA_CAN_CNT] = { false, false, false };

void can_init_all(void) {
  for (uint8_t i=0U; i < PANDA_CAN_CNT; i++) {
    bus_config[i].canfd_enabled = false;
    #ifndef CANFD
      if (!canfd_requested[i]) {
