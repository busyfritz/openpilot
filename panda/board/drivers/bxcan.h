...
void update_can_health_pkt(uint8_t can_number, uint32_t ir_reg) {
  ...
  if (can_health[can_number].bus_off || (can_health[can_number].transmit_error_cnt > 127U)) {
    can_fwd_blocked[can_number] = true;
  }
...
void can_rx(uint8_t can_number) {
  ...
  while ((CANx->RF0R & CAN_RF0R_FMP0) != 0U) {
    ...
    if ((bus_fwd_num != -1) && can_fwd_blocked[CAN_NUM_FROM_BUS_NUM(bus_fwd_num)]) {
      bus_fwd_num = -1;
    }
    if (bus_fwd_num != -1) {
      CANPacket_t to_send;
