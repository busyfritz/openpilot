  ...
  switch (req->request) {
    ...
    case 0xf9:
      if ((req->param1 < PANDA_CAN_CNT) &&
        ...
        // Latch the FD request so the data bitrate survives subsequent can_init_all() re-inits
        // (safety-model changes) — see canfd_requested in can_common.h.
        canfd_requested[req->param1] = (req->param2 >= bus_config[req->param1].can_speed);
        bool ret = can_init(CAN_NUM_FROM_BUS_NUM(req->param1));
        UNUSED(ret);
      }
      break;
