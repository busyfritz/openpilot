...
LIMIT_HOLD_RADIUS_M = 1000.0
LIMIT_HOLD_TIME_S = 60.0
...
class SpeedLimitResolver:
  ...
  def __init__(self):
    ...
    # Distance/time travelled since we last had a valid limit, for held-limit expiry.
    self._dist_since_limit = 0.
    self._time_since_limit = 0.
  ...
  def update_speed_limit_states(self) -> None:
    ...
    if self.speed_limit > 0.:
      ...
      self._dist_since_limit = 0.
      self._time_since_limit = 0.
    elif self.speed_limit_last > 0.:
      # No current limit from any source: keep holding the last one until we've clearly moved on.
      self._dist_since_limit += max(self.v_ego, 0.) * DT_MDL
      self._time_since_limit += DT_MDL
      if self._dist_since_limit > LIMIT_HOLD_RADIUS_M and self._time_since_limit > LIMIT_HOLD_TIME_S:
        self.speed_limit_last = 0.
        self.speed_limit_final_last = 0.
