...
from openpilot.iqpilot.sab.state_machine import GuidanceStateMachine, SOFT_DISABLE_TIME
...
class TestSABStateMachine:
  @pytest.fixture(autouse=True)
  def setup_method(self, mocker: MockerFixture):
    self.guidance = MockSAB(mocker)
    self.state_machine = GuidanceStateMachine(self.guidance)
    self.events = self.guidance.selfdrive.events
    self.events_iq = self.guidance.selfdrive.events_iq
    self.guidance.selfdrive.state_machine.soft_disable_timer = int(SOFT_DISABLE_TIME / DT_CTRL)
  ...
  def test_soft_disable_timer(self):
    ...
    for _ in range(int(SOFT_DISABLE_TIME / DT_CTRL)):
      ...
      self.guidance.selfdrive.state_machine.soft_disable_timer -= 1
      self.state_machine.update()
