...
from openpilot.iqpilot.sab.preferences import DriverInterventionMode, resolve_brake_intervention_mode, \
  BRANDS_WITHOUT_MAIN_CRUISE_TOGGLE, read_aol_enabled_pref, read_main_cruise_pref, read_joint_engagement_pref
from openpilot.iqpilot.sab.state_machine import GuidanceStateMachine, GEARS_ALLOW_PAUSED_SILENT
...
class SteeringAssistanceBehavior:
  ...
  def should_silent_lkas_enable(self, CS: structs.CarState) -> bool:
    if self.steering_mode_on_brake == DriverInterventionMode.SUSPEND and self.pedal_pressed_non_gas_pressed(CS):
      return False
  ...
  def update_events(self, CS: structs.CarState):
    ...
    if not self.selfdrive.enabled and self.enabled:
      ...
      if self.steering_mode_on_brake == DriverInterventionMode.SUSPEND:
    ...
    if self.steering_mode_on_brake == DriverInterventionMode.CANCEL:
      if self.pedal_pressed_non_gas_pressed(CS):
        if self.enabled:
          self.events_iq.add(EventNameIQ.lkasDisable)
        else:
