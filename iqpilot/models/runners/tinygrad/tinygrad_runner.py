...
class TinygradSplitRunner(ModelRunner):
  ...
  def refresh_policy_features(self, features_buffer: np.ndarray) -> None:
    for runner in (self.policy_runner, self.off_policy_runner, self.on_policy_runner):
      if runner is not None and 'features_buffer' in runner.input_to_dtype:
        runner.inputs['features_buffer'] = Tensor(features_buffer,
                                                  device=runner.input_to_device['features_buffer'],
                                                  dtype=runner.input_to_dtype['features_buffer']).realize()

  def _run_model(self) -> NumpyDict:
    vision_output = self.run_vision()
    return {**vision_output, **self.run_policy()}
